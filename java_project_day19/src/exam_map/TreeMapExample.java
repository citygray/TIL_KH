...
public class TreeMapExample {
	...
	public static void main(String[] args) {
		...
		//higherEntry()는 인자로 전달한 키값 다음의 키값을 가진 Entry를 찾아서 반환
		entry = users.higherEntry("20120101");
		print("2012년도에 가장 빠르게 입사한 사람",entry);
		
		//pollFirstEntry()는 Map의 Entry 중 가장 작은 키값을 가진 Entry를 추출한 후 삭제
		//pollLastEntry()는 Map의 Entry 중 가장 큰 키값을 가진 Entry를 추출한 후 삭제
		while(!users.isEmpty()) {
